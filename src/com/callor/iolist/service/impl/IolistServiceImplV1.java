			...
			while(true) {
				...
				try {
					...
					if(strPrice.isBlank()) {
						...
						HelpMessage.ERROR(
								String.format("판매단가는 정수로 입력하세요 (%s)", strPrice)
								);
						continue;
					}
				} // end try
		...
		while(true) {
			...
			try {
				...
				if(result.equals(strDate)) {
					...
					HelpMessage.ERROR(
							String.format("날짜가 유효범위를 벗아났습니다"), strDate);
				}
				
			} catch (ParseException e) {
				HelpMessage.ERROR(
						String.format("날짜 입력 형식 오류(yyyy-MM-dd) : %s", strDate));
