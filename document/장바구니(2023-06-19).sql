...
-- iolist User 가 접근(CRUD 실행) 할 수 있는 Table 을 보여달라
...
INSERT INTO tbl_buyer(buid,buname, butel, buaddr)
VALUES('0000000003','이몽룡', '010-3333-3333','한양특별시');
INSERT INTO tbl_buyer(buid,buname, butel, buaddr)
...
/*
중간문자열 검색
Full Text Serch : 칼럼의 데이터의 문자열을 모두 검색하여 일치하는 데이터 찾기
SELECT 조회 명령 실행 과정에서 가장 느린 Query

칼럼의 데이터 중에 일부라도 일치하는 문자열이 있는 데이터 조회하기
*/
-- 이름 중간 문자열 검색
-- 홍 이라는 문자열로 시작하는 모든 이름
...
그런데, Oracle 에서는 FROM(절) 키워드가 생략된 SELECT 명령을 허용하지 않는다.
